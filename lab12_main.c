...
void __interrupt() isr (void){
    if(PIR1bits.ADIF){                  // Fue interrupción del ADC?
            if(ADCON0bits.CHS == 1){    // Verificamos sea AN0 el canal seleccionado
                contador = ADRESH;         // Mostramos ADRESH en PORTC
                PORTC = contador;
            }
        PIR1bits.ADIF = 0;              // Limpiamos bandera de interrupci?n  
            ...
            else if(!PORTBbits.RB2){               //Si estamos despiertos, sleep
                sleep = 0;                         // despertamos pic     
                write_EEPROM(address, contador);       // escribir en el eeprom 
            }
...
void main(void) {
    ...
    while(1){        
        ...
        if(sleep == 1){               //bandera activada ---> deshabilitar la interrupción ADC        
            PIE1bits.ADIE = 0;
...
void setup(void){
    ...
    TRISB = 0b111;  //para  botones 
    PORTB = 0;
    
    TRISA = 0b10;   //para potenciometro 
    TRISC = 0;      //salida de contador pot
    TRISD = 0;      //para mostrar valor de EEPROM 
